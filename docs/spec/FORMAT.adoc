...
This document and the abcrypt encrypted data format are distributed under the
terms of the {cc0-url}[CC0 1.0 Universal] (CC0 1.0). You can copy, modify,
distribute and perform these, even for commercial purposes, all without asking
permission.

== Introduction

abcrypt is a modern file encryption format inspired by the
{scrypt-encrypted-data-format}[scrypt encrypted data format], which is produced
by the {tarsnap-scrypt}[scrypt encryption utility]. abcrypt uses <<argon2>> for
key derivation, <<blake2b-512-mac>> for header integrity checking and
...
include::example$abcrypt.abnf[lines=5..]
endif::[]
ifndef::site-gen-antora[]
include::abcrypt.abnf[lines=5..]
endif::[]
----
